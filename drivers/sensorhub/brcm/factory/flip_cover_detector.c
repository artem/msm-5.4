					...
					struct device_attribute *attr, char *buf)
{
	...
	if (data->fdc_data.nfc_cover_status == COVER_ATTACH || data->fdc_data.nfc_cover_status == COVER_ATTACH_NFC_ACTIVE) {
		snprintf(sysfs_cover_status, 10, "CLOSE");
	} else if (data->fdc_data.nfc_cover_status == COVER_DETACH) {
				      ...
				      struct device_attribute *attr, const char *buf, size_t size)
{
	...
	data->fdc_data.nfc_cover_status = status;

	msg = kzalloc(sizeof(*msg), GFP_KERNEL);
	msg->cmd = MSG2SSP_AP_SET_FCD_COVER_STATUS;
	msg->length = sizeof(data->fdc_data.nfc_cover_status);
	msg->options = AP2HUB_WRITE;
	msg->buffer = (u8 *)&data->fdc_data.nfc_cover_status;
	msg->free_buffer = 0;
	...
	pr_info("[SSP] %s nfc_cover_status is %d \n", __func__,  data->fdc_data.nfc_cover_status);
...
static void factory_data_init(struct ssp_data *data)
{
	...
	mag_data[X] = data->buf[FLIP_COVER_DETECTOR].uncal_mag_x;
	mag_data[Y] = data->buf[FLIP_COVER_DETECTOR].uncal_mag_y;
	mag_data[Z] = data->buf[FLIP_COVER_DETECTOR].uncal_mag_z;

	pr_info("[SSP] [FACTORY] init data : %d %d %d ", mag_data[X], mag_data[Y], mag_data[Z]);

	factory_data->axis_select = data->fdc_data.axis_update;
	factory_data->threshold = data->fdc_data.threshold_update;
...
void check_cover_detection_factory(struct ssp_data *data)
{
	...
	mag_data[Y] = data->buf[FLIP_COVER_DETECTOR].uncal_mag_y;
	mag_data[Z] = data->buf[FLIP_COVER_DETECTOR].uncal_mag_z;
	factory_data->saturation = data->buf[FLIP_COVER_DETECTOR].saturation;
...
static void enable_factory_test(struct ssp_data *data, int request)
{
	...
	data->fdc_data.factory_cover_status = request;
	msg = kzalloc(sizeof(*msg), GFP_KERNEL);
	msg->cmd = FCD_FACTORY;
	msg->length = sizeof(data->fdc_data.factory_cover_status);
	msg->options = AP2HUB_WRITE;
	msg->buffer = (u8 *)&data->fdc_data.factory_cover_status;
	msg->free_buffer = 0;
...
static ssize_t axis_threshold_setting_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	pr_info("[SSP] [FACTORY] axis=%d, threshold=%d \n", data->fdc_data.axis_update, data->fdc_data.threshold_update);
					    ...
					    struct device_attribute *attr, const char *buf, size_t size)
{
	...
	data->fdc_data.axis_update = axis;
	data->fdc_data.threshold_update = threshold;

	memcpy(shub_data, &data->fdc_data.axis_update, sizeof(data->fdc_data.axis_update));
	memcpy(shub_data + 1, &data->fdc_data.threshold_update, sizeof(data->fdc_data.threshold_update));
