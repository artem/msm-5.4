...
int llc_off_disable(bool off)
{
	...
	sci_data->off_disable_cnt += off ? 1 : -1;
	if (sci_data->off_disable_cnt < 0)
		sci_data->off_disable_cnt = 0;

	if (!off && sci_data->off_disable_cnt) {
		SCI_INFO("skip %s[%d]: off_disable_cnt: %d\n",
				__func__, off, sci_data->off_disable_cnt);
		return 0;
	} else if (off && sci_data->off_disable_cnt > 1) {
		SCI_INFO("skip %s[%d]: off_disable_cnt: %d\n",
				__func__, off, sci_data->off_disable_cnt);
		return 0;
	}
